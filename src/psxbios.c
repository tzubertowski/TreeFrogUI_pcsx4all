...
INLINE void DeliverEvent(u32 ev, u32 spec) {
	...
	/* Mark ready for BOTH modes too: games (e.g. Harvest Moon) open the card
	 * events as callback (EvMdINTR) but also poll them with WaitEvent/TestEvent,
	 * which only succeed on EvStALREADY. */
	Event[ev][spec].status = EvStALREADY;
}

/* --- Deferred (asynchronous) memcard event delivery -------------------------
 * Real PS1 memcard ops complete asynchronously: the BIOS card functions start
 * the op and return, and the completion event is delivered later from the
 * vblank IRQ handler (see Rearmed's card_vint_handler). Games arm their
 * WaitEvent/TestEvent loop AFTER the BIOS call returns, so delivering the event
 * synchronously inside the call (as this HLE used to) makes them miss it and
 * hang. We instead queue the events and deliver them a short time later via the
 * PSXINT_CARD_IO event. */
#define CARD_PENDING_MAX 16
#define CARD_IO_DELAY    20000   /* cycles before delivery (game arms its wait) */
static struct { u8 ev, spec; } card_pending[CARD_PENDING_MAX];
static int card_pending_n = 0;

static void card_defer(u32 ev, u32 spec) {
	if (card_pending_n < CARD_PENDING_MAX) {
		card_pending[card_pending_n].ev   = (u8)ev;
		card_pending[card_pending_n].spec = (u8)spec;
		card_pending_n++;
	}
	psxEvqueueAdd(PSXINT_CARD_IO, CARD_IO_DELAY);
}

/* Dispatched from the event queue (registered in psxevents.c). */
void cardIoDeliver(void) {
	int n = card_pending_n;
	card_pending_n = 0;
	for (int i = 0; i < n; i++)
		DeliverEvent(card_pending[i].ev, card_pending[i].spec);
}
...
void psxBios__card_info(void) { // ab
	...
	/* Async: queue completion + "connected" (0x8000) events, delivered shortly
	 * after this call returns (see card_defer / PSXINT_CARD_IO). */
	card_defer(0x11, 0x2); // 0xf0000011, 0x0004
	card_defer(0x81, ret); // 0xf4000001, 0x0004
	if (ret == 0x2) {
		card_defer(0x11, 0xf); // 0x8000 "connected"
		card_defer(0x81, 0xf);
	}
...
void psxBios__card_load(void) { // ac
	...
	card_defer(0x11, 0x2); // 0xf0000011, 0x0004 — async completion
	card_defer(0x81, 0x2); // 0xf4000001, 0x0004
