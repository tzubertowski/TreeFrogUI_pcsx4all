...
enum psxEventNum {
	...
	PSXINT_CARD_IO,                  // Deferred (async) delivery of memcard BIOS
	                                 //  completion events. Mirrors Rearmed's
	                                 //  card_vint_handler: the game must arm its
	                                 //  WaitEvent/TestEvent loop BEFORE the event
	                                 //  fires — delivering synchronously inside
	                                 //  the BIOS call hangs games (Harvest Moon).
	PSXINT_COUNT,
